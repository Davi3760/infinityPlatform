...
Mario.LevelState.prototype.Update = function(delta) {
    ...
    //Alteracao
    if(Mario.MarioCharacter.X>200 && !flagPassou){
        levelGenerator = new Mario.LevelGenerator(2000, 15);
        this.Level = levelGenerator.CreateLevel(this.LevelType, this.LevelDifficulty,this.Level);
        this.Layer = new Mario.LevelRenderer(this.Level, 2000, 240);
        for (i = 0; i < 2; i++) {
            scrollSpeed = 4 >> i;
            w = ((((2000 * 16) - 2000) / scrollSpeed) | 0) + 2000;
            h = ((((this.Level.Height * 16) - 240) / scrollSpeed) | 0) + 240;
            bgLevelGenerator = new Mario.BackgroundGenerator(2000, h / 32 + 1, i === 0, this.LevelType);
            this.BgLayer[i] = new Mario.BackgroundRenderer(bgLevelGenerator.CreateLevel(), 2000, 240, scrollSpeed);
        }
        flagPassou=true;
        alert('teste');
    }

    /*if(Mario.MarioCharacter.X>300 && !flagMarioX){
        alert('Mario posicao x:'+Mario.MarioCharacter.X);
        flagMarioX = true;
        alert('this.Level.Width'+this.Level.Width);
    }*/
    ...
    for (i = 0; i < this.Sprites.Objects.length; i++) {
        ...
        if (sprite !== Mario.MarioCharacter) {
            ...
            if (xd < -64 || xd > 2000 + 64 || yd < -64 || yd > 240 + 64) {
...
Mario.LevelState.prototype.Draw = function(context) {
    ...
    this.DrawStringShadow(context, "X:"+Mario.MarioCharacter.X.toFixed(2), 0, 2);
